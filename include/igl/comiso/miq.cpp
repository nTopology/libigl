...
namespace comiso {
  ...
  public:
    ...
    IGL_INLINE VertexIndexing(const Eigen::PlainObjectBase<DerivedV> &_V,
                              ...
                              const Eigen::Matrix<int, Eigen::Dynamic, 3> &_Handle_Seams,NTInterrupter* interrupter = nullptr
                              );
  ...
  public:
    ...
    IGL_INLINE PoissonSolver(const Eigen::PlainObjectBase<DerivedV> &_V,
                             ...
                             const Eigen::Matrix<int, Eigen::Dynamic, 1>&_Handle_Singular,
                             const MeshSystemInfo &_Handle_SystemInfo,NTInterrupter* interrupter);
  ...
  public:
    IGL_INLINE MIQ_class(const Eigen::PlainObjectBase<DerivedV> &V_,
                         ...
                         std::vector<int> roundVertices = std::vector<int>(),
                         std::vector<std::vector<int> > hardFeatures = std::vector<std::vector<int> >(),NTInterrupter* interrupter = nullptr);
...
IGL_INLINE igl::comiso::VertexIndexing<DerivedV, DerivedF>::VertexIndexing(const Eigen::PlainObjectBase<DerivedV> &_V,
                                                                   ...
                                                                   const Eigen::Matrix<int, Eigen::Dynamic, 3> &_Handle_MMatch,
                                                                   const Eigen::Matrix<int, Eigen::Dynamic, 1> &_Handle_Singular,
                                                                   const Eigen::Matrix<int, Eigen::Dynamic, 3> &_Handle_Seams,NTInterrupter* interrupter
                                                                   ):
V(_V),
F(_F),
Vcut(_Vcut),
                                                                     ...
                                                                     std::vector<std::vector<int> > hardFeatures)
{
  ...
  if(NTInterrupter::startSection(mInterrupter,.05))return;
  for (size_t i = 0; i < hardFeatures.size(); ++i)
  {
    if(NTInterrupter::wasInterrupted(mInterrupter,(double)i/hardFeatures.size()))return;
  ...
  if(NTInterrupter::startSection(mInterrupter,.1))return;
  ...
  if(NTInterrupter::startSection(mInterrupter,.4))return;
  ...
  if(NTInterrupter::startSection(mInterrupter,.2))return;
  MapCoords();
  if(NTInterrupter::endSection(mInterrupter))return;
...
IGL_INLINE void igl::comiso::PoissonSolver<DerivedV, DerivedF>::FixBlockedVertex()
{
  ...
  {
    if(NTInterrupter::wasInterrupted(mInterrupter,(double)i/Hard_constraints.size()))return;
...
IGL_INLINE void igl::comiso::PoissonSolver<DerivedV, DerivedF>::AddSingularityRound()
{
  for (unsigned int v=0;v<V.rows();v++)
  {
    if(NTInterrupter::wasInterrupted(mInterrupter,(double)v/V.rows()))return;
    if (Handle_Singular(v))
    {
...
IGL_INLINE void igl::comiso::PoissonSolver<DerivedV, DerivedF>::AddToRoundVertices(std::vector<int> ids)
{
  ...
  {
    if(NTInterrupter::wasInterrupted(mInterrupter,(double)i/ids.size()))return;
...
IGL_INLINE void igl::comiso::PoissonSolver<DerivedV, DerivedF>::BuildLaplacianMatrix(double vfscale)
{
  ...
  if(NTInterrupter::startSection(mInterrupter,.25))return;
  igl::grad(Vcut, Fcut, G,mInterrupter);
  if(NTInterrupter::endSection(mInterrupter))return;
  ...
  if(NTInterrupter::startSection(mInterrupter,.25))return;
  igl::slice_into(Cotmatrix, idx,  idx,  Lhs,mInterrupter);
  igl::slice_into(Cotmatrix, idx2, idx2, Lhs,mInterrupter);
  if(NTInterrupter::endSection(mInterrupter))return;
  ...
  if(NTInterrupter::startSection(mInterrupter,.25))return;
  igl::slice_into(rhs1, idx,  1, rhs,mInterrupter);
  igl::slice_into(rhs2, idx2, 1, rhs,mInterrupter);
  if(NTInterrupter::endSection(mInterrupter))return;
...
IGL_INLINE void igl::comiso::PoissonSolver<DerivedV, DerivedF>::MapCoords()
{
  ...
  {
    if(NTInterrupter::wasInterrupted(mInterrupter,(double)f/Fcut.rows()))return;
    for (int k=0;k<3;k++)
    {
...
IGL_INLINE void igl::comiso::PoissonSolver<DerivedV, DerivedF>::BuildSeamConstraintsExplicitTranslation()
{
  ...
  {
    if(NTInterrupter::wasInterrupted(mInterrupter,(double)i/num_cut_constraint))return;
...
IGL_INLINE void igl::comiso::PoissonSolver<DerivedV, DerivedF>::BuildUserDefinedConstraints()
{
  ...
  {
    if(NTInterrupter::wasInterrupted(mInterrupter,(double)i/num_userdefined_constraint))return;
                                                                          ...
                                                                          int localIter)
{
  ...
  for (int k=0; k < Lhs.outerSize(); ++k){
    if(NTInterrupter::wasInterrupted(mInterrupter,(double)k/Lhs.outerSize()))return;
  ...
  for (int k=0; k < Constraints.outerSize(); ++k){
    if(NTInterrupter::wasInterrupted(mInterrupter,(double)k/Constraints.outerSize()))return;
  ...
  {
    if(NTInterrupter::wasInterrupted(mInterrupter,(double)i/((n_integer_vars)*2)))return;
  ...
  COMISO::ConstrainedSolver solver(true,mInterrupter);
  ...
  if(NTInterrupter::startSection(mInterrupter,.7))return;
...
IGL_INLINE igl::comiso::MIQ_class<DerivedV, DerivedF, DerivedU>::MIQ_class(const Eigen::PlainObjectBase<DerivedV> &V_,
                                                                   ...
                                                                   std::vector<int> roundVertices,
                                                                   std::vector<std::vector<int> > hardFeatures,NTInterrupter* interrupter):
V(V_),
F(F_),
mInterrupter(interrupter)
{
  if(NTInterrupter::startSection(interrupter,.1))return;
  igl::cut_mesh(V, F, Handle_Seams, Vcut, Fcut,mInterrupter);
  if(NTInterrupter::endSection(interrupter))return;

  if(NTInterrupter::startSection(interrupter,.1))return;
  igl::local_basis(V,F,B1,B2,B3,mInterrupter);
  if(NTInterrupter::endSection(interrupter))return;
  ...
  if(NTInterrupter::startSection(interrupter,.1))return;
  VertexIndexing<DerivedV, DerivedF> VInd(V, F, Vcut, Fcut, TT, TTi, Handle_MMatch, Handle_Singular, Handle_Seams,interrupter);
  if(NTInterrupter::endSection(interrupter))return;
  ...
  if(NTInterrupter::startSection(interrupter,.2))return;
  PoissonSolver<DerivedV, DerivedF> PSolver(V,
                                            ...
                                            PD2_combed,
                                            Handle_Singular,
                                            VInd.Handle_SystemInfo,interrupter);
  ...
  std::vector<std::vector<int> > hardFeatures, NTInterrupter* interrupter)
{
  ...
  if(NTInterrupter::startSection(interrupter,.9))return;
