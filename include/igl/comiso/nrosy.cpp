...
public:
  // Init
  IGL_INLINE NRosyField(const Eigen::MatrixXd& _V, const Eigen::MatrixXi& _F,NTInterrupter* interrupter = nullptr);
...
igl::comiso::NRosyField::NRosyField(const Eigen::MatrixXd& _V, const Eigen::MatrixXi& _F,NTInterrupter* interrupter)
{
  using namespace std;
  ...
  igl::edge_topology(V,F, EV, FE, EF,interrupter);
  ...
  for(unsigned i=0; i<EV.rows(); ++i){
    if(NTInterrupter::wasInterrupted(interrupter,(double)i/EV.rows()))return;
  ...
  {
    // First edge
    if(NTInterrupter::wasInterrupted(interrupter,(double)fid/F.rows()))return;
...
void igl::comiso::NRosyField::prepareSystemMatrix(const int N)
{
  ...
  {
    if(NTInterrupter::wasInterrupted(mInterrupter,(double)r/id_p.size()))return;
    int eid = id_p[r];
    int i = EF(eid,0);
    int j = EF(eid,1);
...
void igl::comiso::NRosyField::solveRoundings()
{
  ...
  COMISO::ConstrainedSolver cs(true,mInterrupter);
...
void igl::comiso::NRosyField::solve(const int N)
{
  ...
  if(NTInterrupter::startSection(mInterrupter,.25));
  solveRoundings();
  if(NTInterrupter::endSection(mInterrupter));
...
Eigen::MatrixXd igl::comiso::NRosyField::getFieldPerFace()
{
  ...
  for(unsigned i=0; i<F.rows(); ++i){
    if(NTInterrupter::wasInterrupted(mInterrupter,(double)i/F.rows()))return Eigen::MatrixXd();
...
void igl::comiso::NRosyField::computek()
{
  ...
  {
    if(NTInterrupter::wasInterrupted(mInterrupter,(double)eid/EF.rows()))return;
    if (!isBorderEdge[eid])
    {
      int fid0 = EF(eid,0);
      int fid1 = EF(eid,1);
...
IGL_INLINE void igl::comiso::nrosy(
  ...
  const double soft,
  Eigen::MatrixXd& R,
  Eigen::VectorXd& S,NTInterrupter* interrupter)
{
  // Init solver
  if(NTInterrupter::startSection(interrupter,.2))return;
  igl::comiso::NRosyField solver(V,F,interrupter);
  if(NTInterrupter::endSection(interrupter))return;
  
  // Add hard constraints
  if(NTInterrupter::startSection(interrupter,.1))return;
  for (unsigned i=0; i<b.size();++i){
    if(NTInterrupter::wasInterrupted(interrupter,(double)i/b.size()))return;
  ...
  for (unsigned i=0; i<b_soft.size();++i){
    if(NTInterrupter::wasInterrupted(interrupter,(double)i/b_soft.size()))return;
  ...
  // Interpolate
  if(NTInterrupter::startSection(interrupter,.4))return;
  solver.solve(N);
  if(NTInterrupter::endSection(interrupter))return;
