    ...
    NTInterrupter* mInterrupter) 
{
  ...
  VectorXJ  CJ;
  {
      ...
      if (NTInterrupter::startSection(mInterrupter, 0.5)) return false;
      resolve_fun(VV, FF, V, F, CJ);
      if (NTInterrupter::endSection(mInterrupter)) return false;
  }
#ifdef MESH_BOOLEAN_TIMING
  ...
  if (NTInterrupter::startSection(mInterrupter, 0.1)) return false;
...
IGL_INLINE bool igl::copyleft::boolean::mesh_boolean(
  ...
  std::function<void(
    ...
    [&mInterrupter](const Eigen::PlainObjectBase<DerivedVA>& V,
  ...
  return igl::copyleft::boolean::mesh_boolean(
      VA, FA, VB, FB,
      wind_num_op, keep, resolve_func, VC, FC, J, mInterrupter);
}
