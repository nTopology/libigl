    ...
    Veci&      _to_round)
{
  ...
  // check solver performance (only for testing!!!)
  {
    ...
    if(enable_performance_test)
    {
      ...
      Vecd res(_x);
      gmm::add<Vecd>(_x,gmm::scaled(x5,-1.0),res);
      std::cerr << "DIFFERENCE IN RESULT: " << gmm::vect_norm2(res) << std::endl;
    }
#endif
  }
...
MISolver::update_solution( 
    ...
    Vecd&      _x, 
    Vecd&      _rhs, 
    Vecui&     _neigh_i, NTInterrupter* interrupter )
{
  ...
  if(max_local_iters_ > 0)
  {
    ...
    converged = siter_.gauss_seidel_local(_A, _x, _rhs, _neigh_i, n_its, tolerance,interrupter);
  ...
  if( !converged && max_cg_iters_ > 0)
  {
    ...
    converged = siter_.conjugate_gradient(_A, _x,_rhs, max_cg_iters, tolerance,interrupter);
...
MISolver::solve_multiple_rounding( 
    ...
    Vecd&      _x, 
    Vecd&      _rhs, 
    Veci&      _to_round,NTInterrupter* interrupter)
{
  // StopWatch
  COMISO::StopWatch sw;
  double time_search_next_integer = 0;
  ...
  if(NTInterrupter::startSection(interrupter,.1))return;
  for(unsigned int i=0; i<old_idx.size(); ++i){
    if(NTInterrupter::wasInterrupted(interrupter,(double)i/old_idx.size()))return;
    old_idx[i] = i;
  }
  if(NTInterrupter::endSection(interrupter))return;
  ...
  for(unsigned int i=0; i<to_round.size(); ++i)
  {
    if(NTInterrupter::wasInterrupted(interrupter,(double)i/to_round.size()))return;
    if( noisy_ > 0)
    {
      std::cerr << "Integer DOF's left: " << to_round.size()-(i+1) << " ";
      if( noisy_ > 1)
    ...
    // 3-stage update of solution w.r.t. roundings
    // local GS / CG / SparseCholesky
    update_solution( _A, xr, _rhs, neigh_i,interrupter);
    if(NTInterrupter::wasInterrupted(interrupter))return;
  }
  if(NTInterrupter::endSection(interrupter))return;
